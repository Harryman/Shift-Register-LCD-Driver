...
uint8_t _pinmap[5][9] = {
  {0,2,0,0,0,0,0,3,0}, // digit:0  a,b,c,d,e,f,g,dp,col
  {34,33,6,5,4,35,36,7,32}, // digit:1  a,b,c,d,e,f,g,dp,col
  {29,28,10,9,8,30,31,11,27}, // digit:2  a,b,c,d,e,f,g,dp,col
  {24,23,14,13,12,25,26,15,0}, // digit:3  a,b,c,d,e,f,g,dp,col
  {20,19,18,17,16,21,22,0,0}// digit:4  a,b,c,d,e,f,g,dp,col
}; 
...
void shiftLCD::print(char *str, uint8_t offset){
  ...
  for (int x = offset; (str[x] != '\0') && ((x-offset)<(digits-halfD)); x++){
    if(str[x] == 46 || str[x] == 58){
      ...
      setChar(str[x], digit + halfD); //starts at the first whole digit
      digit++;
      }
    }
  }
...
void shiftLCD::printFloat(float number, uint8_t decPlaces){//Displays float with specified number of decimal places, will move decimal when truncating
  ...
  setArray(_pinmap[((digits - 1)-decPlaces)][7]);
}
...
void shiftLCD::setSeg(char seg, uint8_t digit){
  if(seg>96 && seg<104 && digit<digits && digit>=0){
    setArray(_pinmap[digit][seg-97]);
  }
}
